...
def draw_collision(arbiter, space, data):
    if True: # Andrew trace attention objects
    ...
    while running:
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                running = False
            elif event.type == KEYDOWN and event.key == K_a:  # Andrew
                attention = Object()
                attention.time = [0]
                attention.number = [0]
                attention.impulces = [[]]
            ...
            elif event.type == KEYDOWN and event.key == K_h:  # Andrew
                shape.body._set_velocity((0.0, -500.0))
            elif event.type == KEYDOWN and event.key == K_i:  # Andrew input value of x coordinate of a new boal
                new_ball_x = raw_input("Enter new ball X coordinate: ")
                trg_new_ball_x = True
            elif event.type == KEYDOWN and event.key == K_n: #Andrew
                trg_next_ball = True
                ...
                if trg_Pause:
                    ...
                    trg_Pause = True
                    print('K_p > True')
            elif event.type == KEYDOWN and event.key == K_r: #Andrew
                trg_restart = True
            elif event.type == KEYDOWN and event.key == K_s:  # Andrew
                trg_simulation_newBatch = True
                print('K_s')
            elif event.type == KEYDOWN and event.key == K_v:  # Andrew
                trg_K_v = True
                print('K_v')
            elif event.type == KEYDOWN and event.key == K_w:  # Andrew - write
                print('K_v')
            ...
            elif event.type == KEYDOWN and event.key == K_z:  # Andrew - execute a solution
        ...
        if not trg_Pause:
            ...
            for ball in balls:
                ...
                if 'line' in dir(ball): #Andrew > save a trajectory in the ball object
            ...
            for vert_id in range(0,len(verticies)): # Andrew
                pGD[vert_id] = tuple(map(lambda x, y: x + y, verticies[vert_id], pG))
            pygame.draw.polygon(screen, THECOLORS["red"], pGD, 2) # Andrew draw goal object
